...
function checkSelf (idealTree, pkg, force, next) {
  ...
  if (force) {
    ...
    var er = new Error('Refusing to install package with name "' + pkg.name + '" under a package also called "' + pkg.name + '". Did you name your project the same as the dependency you\'re installing?')
    er.code = 'ENOSELF'
    next(er)
  }
}
