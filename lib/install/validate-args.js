...
function checkSelf (idealTree, pkg, force, next) {
  ...
  if (force) {
    ...
    var er = new Error('Refusing to install package with name "' + pkg.name + '" under a package also called "' + pkg.name + '". Did you name your project the same as the dependency you\'re installing? For more information, see https://docs.npmjs.com/cli/install#limitations-of-npms-install-algorithm')
    er.code = 'ENOSELF'
    next(er)
  }
}
